...
**Licence: GPL-3.0. This is a fork of `dsp56300/dsp56300`.** Never open a pull
request against that repository.

This project's work is `Peripherals56311` and its tests. Most of the tree is
vendored wxWidgets and the VTune SDK. Never lint, format, or refactor vendored
code, and never remove a comment that came with it.

Build and test:

    cmake -G Ninja -B build -DCMAKE_BUILD_TYPE=Release -DBUILD_TESTING:BOOL=true
    cmake --build build
